...
static void on_setup(void* ctx) {
    app_t* app = (app_t*)ctx;
    mc_animate_init_spring(&app->anim_x, MC_FP_C(400), MC_FP_C(400));
    mc_animate_init_spring(&app->anim_y, MC_FP_C(225), MC_FP_C(225));
    app->anim_x.config.spring.stiffness = MC_FP_C(120);
    app->anim_x.config.spring.damping = MC_FP_C(12);
...
int main(void) {
    app_t app;
    mc_example_run(800, 450, "Animate Value - C99", on_setup, on_draw, &app);
    return 0;
}
